...
def group_search_async(
    ...
    """Check group `groupio` for an existing, unregistered `req.file`.
...
class DefaultGroupIO(BaseGroupIO):
    ...
    def __init__(
        self, group: StorageGroup, config: dict, queue: FairMultiFIFOQueue, fifo: Hashable
    ) -> None:
    ...
    def nodes(self, nodes: list[UpdateableNode]) -> None:
        ...
        nodes : list of UpdateableNodes
            This ill always be a single-element list containing the
            group's `StorageNode`.
