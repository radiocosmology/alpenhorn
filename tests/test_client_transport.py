...
def test_mount_transport(mock, mock_ismount, mock_node_check, fixtures):
    ...
    help_result = runner.invoke(cli.cli, ['mount-transport', '--help'])
    ...
    result = runner.invoke(cli.cli, args=['mount-transport', 'z'])
    ...
    result = runner.invoke(cli.cli, args=['mount-transport', 'x'])
    assert result.exit_code == 0
    assert mock.mock_calls == [call('mount /mnt/z')]
...
def test_unmount_transport(mock, fixtures):
    ...
    help_result = runner.invoke(cli.cli, ['unmount-transport', '--help'])
    assert help_result.exit_code == 0
    ...
    result = runner.invoke(cli.cli, args=['unmount-transport', 'x'])
    assert result.exit_code == 0
...
def test_format_transport(getuid_mock, glob_mock,
    ...
    help_result = runner.invoke(cli.cli, ['format-transport', '--help'])
    assert help_result.exit_code == 0
    assert 'Interactive routine for formatting a transport disc as a storage node' in help_result.output
    assert 'Options:\n  -h, --help  Show this message and exit.' in help_result.output

    result = runner.invoke(cli.cli, args=['format-transport', '12-34-56-78'])
    assert result.exit_code == 0
