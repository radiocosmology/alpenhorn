...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers",
        "lfs_hsm_state(dict): "
        "used on tests which mock alpenhorn.daemon.io.lfs.LFS "
...
def mock_filesize():
    ...
    with patch("alpenhorn.daemon.io.default.DefaultNodeIO.filesize", _mock_filesize):
        yield
...
def mock_lfs(have_lfs, request):
    ...
    from alpenhorn.daemon.io.lfs import LFS, HSMState
    ...
    if "hsm_state" not in lfs_dont_mock:
        patches.append(
            patch("alpenhorn.daemon.io.lfs.LFS.hsm_state", _mocked_lfs_hsm_state)
        )
    if "hsm_release" not in lfs_dont_mock:
        patches.append(
            patch("alpenhorn.daemon.io.lfs.LFS.hsm_release", _mocked_lfs_hsm_release),
        )
    if "hsm_restore" not in lfs_dont_mock:
        patches.append(
            patch("alpenhorn.daemon.io.lfs.LFS.hsm_restore", _mocked_lfs_hsm_restore),
        )
    if "quota_remaining" not in lfs_dont_mock:
        patches.append(
            patch(
                "alpenhorn.daemon.io.lfs.LFS.quota_remaining",
                _mocked_lfs_quota_remaining,
            ),
        )
...
def mock_observer():
    ...
    with patch("alpenhorn.daemon.io.default.DefaultNodeIO.observer", PollingObserver):
        yield
