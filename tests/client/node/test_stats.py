...
def test_no_list(clidb, client):
    ...
    result = client(0, ["node", "stats"])

    # i.e. the header hasn't been printed
    assert "Count" not in result.output


def test_list(some_nodes, client, assert_row_present):
...
def test_active(some_nodes, client, assert_row_present):
    """Test limit --active."""

    result = client(0, ["node", "stats", "--active"])
    assert_row_present(result.output, "Node1", 2, "3.000 GiB", "30.00")
...
def test_inactive(some_nodes, client, assert_row_present):
    """Test limit --inactive."""

    result = client(0, ["node", "stats", "--inactive"])
...
def test_host(some_nodes, client, assert_row_present):
    """Test limit --host."""

    result = client(0, ["node", "stats", "--host=Host2"])
    assert "Node1" not in result.output
    assert "Node2" not in result.output
    assert_row_present(result.output, "Node3", 0, "-", "-")
    assert_row_present(result.output, "Node4", 1, "1.000 GiB", "5.00")
...
def test_group(some_nodes, client, assert_row_present):
    """Test limit --group."""

    result = client(0, ["node", "stats", "--group=Group1"])
    assert_row_present(result.output, "Node1", 2, "3.000 GiB", "30.00")
    assert_row_present(result.output, "Node2", 0, "-", "-")
    assert_row_present(result.output, "Node3", 0, "-", "-")
    assert "Node4" not in result.output
...
def test_limit_bad_group(some_nodes, client):
    """Test limit matching to a non-existent group."""

    result = client(1, ["node", "stats", "--group=Group3"])
...
def test_limit_nothing(some_nodes, client):
    """Test limit matching nothing."""

    result = client(0, ["node", "stats", "--host=Host3"])
...
def test_extra(some_nodes, client, assert_row_present):
    """Test --extra-stats."""

    result = client(0, ["node", "stats", "--extra-stats"])
