...
def test_no_node(clidb, client):
    ...
    client(1, ["node", "activate", "TEST"])


def test_activate_default(clidb, client):
...
def test_activate_set(clidb, client):
...
def test_activate_clear(clidb, client):
...
def test_already_active(clidb, client):
