...
def test_active(some_nodes, client, assert_row_present):
    ...
    result = client(0, ["node", "list", "--active"])
...
def test_inactive(some_nodes, client, assert_row_present):
    ...
    result = client(0, ["node", "list", "--inactive"])
    assert_row_present(result.output, "Node1", "Group1", "-", "", "", "No", "", "")
...
def test_host(some_nodes, client, assert_row_present):
    ...
    result = client(0, ["node", "list", "--host=Host2"])
    assert "Node1" not in result.output
    assert_row_present(
        result.output,
        "Node2",
...
def test_group(some_nodes, client, assert_row_present):
    ...
    result = client(0, ["node", "list", "--group=Group1"])
    assert_row_present(result.output, "Node1", "Group1", "-", "", "", "No", "", "")
    assert_row_present(
        result.output,
        "Node2",
...
def test_limit_bad_group(some_nodes, client):
    ...
    result = client(1, ["node", "list", "--group=Group3"])
...
def test_limit_nothing(some_nodes, client):
    ...
    result = client(0, ["node", "list", "--host=Host3"])
