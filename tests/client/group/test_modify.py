...
def test_no_modify(clidb, client):
    """Test modifying a non-existent group."""

    client(1, ["group", "modify", "TEST", "--notes=Notes"])

    # Still nothing.
    assert StorageGroup.select().count() == 0


def test_modify_empty(clidb, client):
    ...
    client(0, ["group", "modify", "TEST"])


def test_modify_no_change(clidb, client):
    """Test modify with no change."""

    StorageGroup.create(name="TEST", notes="Note", io_class=None)

    client(0, ["group", "modify", "TEST", "--notes=Note"])

    assert StorageGroup.get(name="TEST").notes == "Note"


def test_modify(clidb, client):
    """Test modify."""

    StorageGroup.create(name="TEST", notes="Note", io_class=None)

    client(0, ["group", "modify", "TEST", "--notes=New Note", "--class=NewClass"])
...
def test_modify_delete(clidb, client):
    """Test deleting metadata with modify."""

    StorageGroup.create(name="TEST", notes="Note", io_class=None)

    client(0, ["group", "modify", "TEST", "--notes=", "--class="])
...
def test_modify_iovar_bad_json(clidb, client):
    ...
    client(1, ["group", "modify", "TEST", "--io-var=a=9"])
...
def test_modify_fix_json(clidb, client):
    ...
    client(0, ["group", "modify", "TEST", '--io-config={"a": 10}'])
