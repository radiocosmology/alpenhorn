...
def test_create(clidb, client):
    """Test creating a group."""

    client(0, ["group", "create", "TEST", "--notes=Notes"])
...
def test_create_existing(clidb, client):
    """Test creating a group that already exists."""

    # Add the group
    StorageGroup.create(name="TEST")

    client(1, ["group", "create", "TEST", "--notes=Notes"])
...
def test_create_ioconifg(clidb, client):
    ...
    client(0, ["group", "create", "TEST", '--io-config={"a": 3, "b": 4}'])
